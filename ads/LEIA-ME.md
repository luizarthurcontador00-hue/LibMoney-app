# Criativos de tráfego pago — LibMoney

Tudo em arquivo final, sem marca d'água, pronto para subir no gerenciador.

- **Estáticos:** 1080 x 1350 (4:5), PNG.
- **Reels:** 1080 x 1920 (9:16), MP4 H.264, 30 fps, **sem áudio** (para poder
  entrar música na hora de subir, ou rodar mudo com legenda).
- **CTA único em tudo:** *Teste 7 dias grátis* + *Depois, R$ 9,99/mês*.
  Nenhuma peça diz "baixe grátis".

---

## Estáticos — 6 conceitos x 3 headlines

Mesmo layout, mesmo print e mesmo CTA dentro de cada conceito: a única coisa que
muda entre 1, 2 e 3 é a headline. É o que permite ler o teste A/B.

### A — Fluxo diário (fundo claro)
| Arquivo | Headline |
|---|---|
| `ad_A1.png` | Seu salário **some** antes do fim do mês? |
| `ad_A2.png` | Para onde foi o dinheiro **desta semana**? |
| `ad_A3.png` | Descubra **em que dia** o mês começou a apertar. |

### B — Projetado x Real (fundo verde escuro)
| Arquivo | Headline |
|---|---|
| `ad_B1.png` | O que você planejou. O que **aconteceu**. |
| `ad_B2.png` | Onde o seu mês **saiu do plano**. |
| `ad_B3.png` | Orçamento não é palpite. É **comparação**. |

### C — Visão geral (fundo claro)
| Arquivo | Headline |
|---|---|
| `ad_C1.png` | Todas as suas contas em **uma tela**. |
| `ad_C2.png` | Quanto você **realmente** tem hoje? |
| `ad_C3.png` | Pare de somar **saldo na cabeça**. |

### D — Lançamentos (fundo verde escuro)
| Arquivo | Headline |
|---|---|
| `ad_D1.png` | Anotar um gasto leva **segundos**. |
| `ad_D2.png` | Sem planilha. Sem **enrolação**. |
| `ad_D3.png` | Simples o bastante para **usar todo dia**. |

### E — Preço (fundo verde escuro, sem print)
| Arquivo | Headline |
|---|---|
| `ad_E1.png` | Controle das suas finanças por **menos que um lanche**. |
| `ad_E2.png` | Menos de **R$ 0,35 por dia** para saber onde está o seu dinheiro. |
| `ad_E3.png` | Quanto vale saber para onde vai **o seu dinheiro**? |

### F — Resultado do mês (fundo claro)
| Arquivo | Headline |
|---|---|
| `ad_F1.png` | Quanto **sobrou** no seu mês? |
| `ad_F2.png` | Fechou o mês **no azul** ou no vermelho? |
| `ad_F3.png` | O mês inteiro resumido em **três números**. |

---

## Reels

| Arquivo | Duração | Trechos usados |
|---|---|---|
| `reel_planejado.mp4` | 11,4 s | Projetado x Real |
| `reel_fluxo.mp4` | 12,5 s | Fluxo diário + troca de mês |
| `reel_curto.mp4` | 10,6 s | Fluxo + Projetado x Real + troca de mês |

Os três terminam no mesmo cartão de marca (logo, *Teste 7 dias grátis*, preço).

**Texto de cada Reel**

*reel_planejado* — "Todo mês você faz um plano." → "E todo mês ele escapa." →
"O LibMoney mostra onde escapou."

*reel_fluxo* — "Para onde foi o dinheiro do mês?" → "Dia por dia: entrada, saída
e saldo." → "E os próximos meses já aparecem."

*reel_curto* — "Suas finanças em uma tela só." → "Planejado e real, lado a lado."
→ "O mês que vem já aparece."

---

## Texto principal sugerido (campo de cima do anúncio)

**1.** Você ganha, paga as contas e no dia 20 não sobra nada — mas não sabe
explicar por quê. O LibMoney mostra dia por dia para onde o dinheiro foi, e
compara o que você planejou com o que realmente gastou. Teste 7 dias grátis.

**2.** Controlar dinheiro não precisa de planilha. Lançou receita, despesa,
gasto no cartão ou transferência em segundos — e o app fecha o mês para você.
7 dias grátis, depois R$ 9,99/mês.

**3.** Contas, cartões, faturas e objetivos em um app só. Saldo do dia, do mês e
dos próximos meses. Disponível para Android. Teste 7 dias grátis.

---

## Fora deste pacote, de propósito

- Nenhum nome de pessoa, em nenhuma peça.
- Nenhum número apresentado como resultado de usuário: todos os valores nas
  telas são da conta de teste e aparecem só como interface do produto.
- Nada de WhatsApp ou IA.
- A tela de Objetivos ficou de fora: o único objetivo cadastrado está em 0%,
  e como prova de produto isso joga contra.
